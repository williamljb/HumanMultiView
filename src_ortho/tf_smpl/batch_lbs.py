...
def batch_rodrigues_back(rot, name=None):
    with tf.name_scope(name, "batch_rodrigues_back", [rot]):
        tmp = (rot - tf.matrix_transpose(rot)) / 2
        v = tf.gather(tf.reshape(tmp, [-1, 9]), [7,2,3], axis=1)
        sin = tf.expand_dims(tf.norm(v, axis=1), -1)
        cos = (tf.expand_dims(rot[:,0,0]+rot[:,1,1]+rot[:,2,2], -1) - 1) / 2
        angle = tf.atan2(sin, cos)
        v = v / sin * angle
        return v
