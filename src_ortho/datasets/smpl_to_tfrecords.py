...
def process_smpl_mocap(all_pkls, out_dir, num_shards, dataset_name):
    ...
    for pkl in all_pkls:
        ...
        num_frame = 250
        print(tmp.shape)
        if tmp.shape[0] < num_frame * step:
            continue
        with open(join(out_dir, 'pose%04d.txt'%st), 'w') as f:
            for i in range(25):
                pre = tmp[0] / 25. * i
                pre[:3] = (0,0,0)
                for j in range(pre.shape[0]):
                    f.write('{},'.format(pre[j]))
                f.write('\n')
            for i in range(num_frame):
                tmp[i*step,:3] = cv2.Rodrigues(np.matmul(
                    cv2.Rodrigues(rev)[0],
                    cv2.Rodrigues(tmp[i*step,:3])[0]
                    ))[0][:,0]
                for j in range(tmp.shape[1]):
                    f.write('{},'.format(tmp[i*step,j]))
                f.write('\n')
        st += 1
...
def main(unused_argv):
    ...
    all_pkl = ['/scratch1/hmr_multiview/neutrMosh/neutrSMPL_CMU/08/08_04.pkl']
