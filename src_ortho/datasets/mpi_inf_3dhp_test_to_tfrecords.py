...
tf.app.flags.DEFINE_string('split', 'test', 'train or val')
...
def get_all_data(base_dir, sub_id, seq_id, cam_ids):
    data_dir = join(base_dir, 'TS%d' % sub_id)
    anno_path = join(data_dir, 'annot_data.mat')
    img_dir = join(data_dir, 'imageSequence')
    # Get data for all cameras.
    import h5py
    res = h5py.File(anno_path, 'r')

    valid_frame = np.array(res['valid_frame'], np.int32)[:,0].astype(bool)
    univ_annot3 = np.array(res['univ_annot3'])
    print("valid_frame:",valid_frame.shape)
    print("annot3:",univ_annot3.shape)

    all_gt2ds, all_gt3ds, all_img_paths = [], [], []
    all_cams = []
    for cam_id in cam_ids:
        base_path = join(img_dir, 'crop_%06d.jpg')
        num_frames = univ_annot3.shape[0]
        gt2ds = np.zeros((num_frames, 14, 2), np.float32)
        gt3ds = univ_annot3.reshape(num_frames, -1, 3)
        # Convert N x 28 x . to N x 14 x 2, N x 14 x 3
        gt3ds = gt3ds[:, test_idx2lsp, :]
        img_paths = [base_path % (frame + 1) for frame in range(num_frames)]
        if gt3ds.shape[0] != len(img_paths):
            print('Not same paths?')
            import ipdb
            ipdb.set_trace()
        use_these = valid_frame
        all_gt2ds.append(gt2ds[use_these])
        all_gt3ds.append(gt3ds[use_these])
        cams = np.zeros((gt2ds[use_these].shape[0], 3), np.float32)
        all_cams.append(cams)
        all_img_paths.append(np.array(img_paths)[use_these].tolist())

    all_gt2ds = np.transpose(np.array(all_gt2ds), [1,0,2,3])
    print(all_gt2ds.shape)
...
def add_to_tfrecord(im_paths,
    ...
    for path, gt2d, cam in zip(im_paths, gt2ds, cams):
        ...
        min_pt = np.array([0,0], np.float32)
        max_pt = np.array([300,300], np.float32)
...
def process_mpi_inf_3dhp_train(data_dir, out_dir):
    ...
    for sub_id in sub_ids:
        for seq_id in seq_ids:
            ...
            img_paths, gt2ds, gt3ds, cams = get_all_data(
                data_dir, sub_id, seq_id, cam_ids)
...
def main(unused_argv):
    ...
    process_mpi_inf_3dhp_train(
        FLAGS.data_directory, FLAGS.output_directory)
