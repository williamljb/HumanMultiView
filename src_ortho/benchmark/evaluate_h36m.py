...
kTFDataDir = '/scratch1/williamljb/hmr_multiview/tf_datasets/human36m_wjoints'
...
def get_data(seq_name, config):
    ...
    images, kps, gt3ds, _, _ = read_images_from_tfrecords(
        tf_path, 1, img_size=config.img_size, sess=sess)
...
def run_model(images, config):
    ...
    num_views = 4
    if model is None:
        model = RunModel(config, num_views, 1, sess=sess)
    ...
    for b in xrange(num_total_batches):
        ...
        if end_ind > N:
            ...
            images_wdummy = np.vstack([
                images_here,
                np.zeros((batch_size - num_here, 1, config.img_size,
                          config.img_size, 3))
            ])
            joints, verts, cams, joints3d, thetas = model.predict(
                images_wdummy, get_theta=True)
            joints = joints[0][:num_here]
            verts = verts[0][:num_here]
            cams = cams[0][:num_here]
            joints3d = joints3d[0][:num_here]
            thetas = thetas[0][:num_here]
        else:
            joints, verts, cams, joints3d, thetas = model.predict(
...
def evaluate_sequence(seq_info, pred_dir):
    ...
    if exists(save_path):
        ...
        tot_sec += t1-t0
        tot_num += len(results['verts'])
        # Evaluate!
        # Joints 3D is COCOplus format now. First 14 is H36M joints
        pred3ds = np.expand_dims(results['joints3d'][:, :14, :], 1)
        # Convert to mm!
...
def main(config):
    ...
    for itr, seq_info in enumerate(all_pairs):
        ...
        #print('avg:{}/{}={}'.format(tot_sec, tot_num, tot_sec/tot_num))
        print('====================')
