...
def draw_content(content, config, ind):
    ...
    img = img#vis_util.draw_skeleton(img, kp, draw_edges=False)
    ...
    # plt.axis('off')
    # plt.show()
    # io.imsave('ori{}.png'.format(ind),img / 255)
    # io.imsave('ours{}.png'.format(ind), rend_img / 255)
...
def get_data(seq_name, config):
    ...
    ks = [-1,view]+list(kps.shape[2:])
    gs = [-1,view]+list(gt3ds.shape[2:])

    return images.reshape(ims), kps.reshape(ks), gt3ds.reshape(gs)
